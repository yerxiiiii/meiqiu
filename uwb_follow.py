...
# 摇杆输出限制 (-1.0 ~ 1.0) —— 日志显示摔倒主因是满幅转向+前进
FORWARD_MIN = 0.12
FORWARD_MAX = 0.25   # 再降；稳定后再调大
ROTATE_MIN = 0.12
ROTATE_MAX = 0.22    # 原 0.5 对应 wz≈0.78，边冲边拧极易倒

# 角度与距离控制比例
ANGLE_DEADZONE = 12.0  # 度：噪声大，死区加大
ANGLE_SCALE = 60.0     # 度/满量程：同样偏差输出更小
DISTANCE_SCALE = 120.0 # cm/满量程

# 角度滤波 / 跳变抑制（日志里 ang 曾在 0.1s 内 -34→0→63）
ANGLE_EMA_ALPHA = 0.25          # 越小越稳
ANGLE_JUMP_REJECT_DEG = 35.0    # 单帧跳变超过此值则丢弃该帧角度
ANGLE_HOLD_ON_JUMP = True

# 大转向时压低前进（避免边转边冲）
TURN_FORWARD_COUPLING = 0.75    # rot 满幅时 fwd 最多保留 (1-0.75)=25%
...
USE_OBSTACLE_SIDESTEP = True      # 使用感知给出的 rotate_bias
